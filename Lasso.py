...
class LSTM_Math_Optimizee_Model(nn.Module):
    ...
    def Output_Gradient_Increment_And_Update_LSTM_Hidden_State(self, x, input_gradients, prev_state):
        if prev_state is None: #init_state
            prev_state = (torch.zeros(self.num_stacks,self.batchsize,self.hidden_size),
                            torch.zeros(self.num_stacks,self.batchsize,self.hidden_size))
            if USE_CUDA :
    ...
    def forward(self, x, input_gradients, prev_state):
        ...
        update , next_state = self.Output_Gradient_Increment_And_Update_LSTM_Hidden_State(x, input_gradients , prev_state)
...
class GRU_Math_Optimizee_Model(nn.Module):
    ...
    def Output_Gradient_Increment_And_Update_GRU_Hidden_State(self, x, input_gradients, prev_state, i):
        ...
        lr = self.learn_rate(initial_lr = 0.000001, gamma = 0.96, step = i)#0.00005
        ...
        Z_input = GRU_Math_learner.get_var('Z') 
        if Z_input.grad is not None:
            Z_grad = torch.clone(Z_input.grad).detach()
        else:
            Z_grad = torch.zeros_like(Z_input)
        ...
        P_in = P * lr
        prox_out = GRU_Math_learner.prox({'P':P_in , 'X':prox_in}, compute_grad = True)   
        prox_diff = prox_out - x
        Z_new = prox_out + A * prox_diff
        update = prox_out
        
        GRU_Math_learner.set_var('Z', Z_new)
        return update, next_state
...
def evaluate(best_sum_loss, best_final_loss, best_flag, lr):
    ...
    losses, sum_loss = learner()
    try:
        best = torch.load('LASSO/LSTM_Math_best_loss.txt')
    except IOError:
    ...
    if losses[-1] < best_final_loss and  sum_loss < best_sum_loss:
        ...
        torch.save(LSTM_Math_Optimizee.state_dict(),'LASSO/LSTM_Math_best_optimizer.pth')
        torch.save([best_sum_loss ,best_final_loss,lr ],'LASSO/LSTM_Math_best_loss.txt')
        best_flag = True
...
def evaluate(best_sum_loss, best_final_loss, best_flag, lr):
    ...
    losses, sum_loss = learner()
    try:
        best = torch.load('LASSO/GRU_Math_best_loss.txt')
    except IOError:
    ...
    if losses[-1] < best_final_loss and  sum_loss < best_sum_loss:
        ...
        torch.save(GRU_Math_Optimizee.state_dict(),'LASSO/GRU_Math_best_optimizer.pth')
        torch.save([best_sum_loss ,best_final_loss,lr ],'LASSO/GRU_Math_best_loss.txt')
        best_flag = True
